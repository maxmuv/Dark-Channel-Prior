...
2. Аугментированные с помощью приближенных карты глубины(47 изображений). Данные взяты с сайта (https://unsplash.com/). Для создания карт глубины использовался скрипт *horizon_mapper.py*

3. Реальные данные(45 картинок). Датасет - O-HAZE [3]. Его сайт - https://data.vision.ee.ethz.ch/cvl/ntire18/o-haze/ . Плюсом является то, что в нем имеется ground truth, потому что для создания тумана использовалась специальная машина для создания тумана и снимки были сделаны до и после ее использования. Для переименовывания файлов использовался скрипт *o-haze_renamer.py*.
...
После аугментации и удаления тумана. Был запущен скрипт *comparator.py* на папке images(изображения без тумана) и dcp(изображения с удаленным dcp туманом). Получились следующие результы:
